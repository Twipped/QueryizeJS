...
query.columns = function columns () {
	...
	args = args.map(function (column) {
		...
		if (isQueryizeObject(column)) {
			...
			return self._mergeSubquery(column);
		}
...
/**
 * Compiles an external query object and merges the bound data values into this query
 * @private
 * @memberOf query
 * @param  {query} query
 ...
 *   });
 * @example
 * // query exec used as event emitter
 * queryize()
 *   .select()
 *   .from('employees')
 *   .orderBy('lastname', 'firstname')
 ...
 *     });
 * @example
 * // Query exec piped to node stream
 * queryize()
 *   .select()
 *   .from('employees')
 *   .orderBy('lastname', 'firstname')
